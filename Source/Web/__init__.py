...
def load_assets(assets, app):
    ...
    with app.app_context():
        new_assets = []
        current_db_assets = [
            x.name for x in db.session.query(TargetAsset).all()]
        current_db_network_ids = [
            x.id_string for x in db.session.query(TargetNetworkID).all()]
        for asset in assets:
            try:
                ...
                logger.error(str(integrity_error))
                db.session.rollback()
